class WeatherApp {
    ...
    sendRequest(city) {
        ...
        const format ="%22)%20and%20u%3D'c'&format=json"
    ...
    displayCurrentWeather(city) {
        ...
        if (this.cityWeather[city]) {
            ...
            tempE.textContent = (this.cityWeather[city].current.temp) + 'C'
    ...
    displayForecast(forecastArray) {
        ...
        for(let i=0; i<forecastArray.length; i++) {
            ...
            highTempE.textContent = forecastArray[i].high + 'C'

            const lowTempE = forecastElements[i].querySelector('.lowtemp')
            lowTempE.textContent = forecastArray[i].low + 'C'

            const descriptionE = forecastElements[i].querySelector('.description')
